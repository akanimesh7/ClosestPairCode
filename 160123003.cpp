...
void qsy(pr arr[],int l,int r){
	...
	for(int i=0;i<(hi-lo+1);i++){
		...
		}else if(arry[i].fx() == pvtx && arry[i].fy()<=pvty){
			yleft[cl].sx(arry[i].fx());yleft[cl].sy(arry[i].fy());cl++;
...
public:
	solution(int a,char c,char d){
		...
		for(int i=0;i<t;i++){
			...
			arry[i].sx(arrx[i].fx());
			arry[i].sy(arrx[i].fy());
			//arrx[i].sx(a);arrx[i].sy(b);arry[i].sx(a);arry[i].sy(b);
			if(c=='y'){
				cout<<arrx[i].fx()<<"    "<<arrx[i].fy()<<endl;
			}
		}else{
			double p,q;cin>>p>>q;
			arrx[i].sx(p);arrx[i].sy(q);
			arry[i].sx(p);arry[i].sy(q);
		}
		}
...
int main(){
	...
	char c;cin>>c;
	cout<<"Do you want to enter the points ('e' for enter/'r' for random) ?"<<endl;
	char d;cin>>d;	
	clock_t start=clock();
	solution finalans(t,c,d);
	clock_t end=clock();
	cout<<endl<<"Time elapsed is "<<1000*((double)(end-start))/CLOCKS_PER_SEC<<" ms"<<endl<<endl;
